...
void leb_MergeNode(in const cbt_Node node);
#endif

// Subdivision routine O(depth)
float3   leb_DecodeAttributeArray(in const cbt_Node node, in const float3 data);
float2x3 leb_DecodeAttributeArray(in const cbt_Node node, in const float2x3 data);
float3x3 leb_DecodeAttributeArray(in const cbt_Node node, in const float3x3 data);
float4x3 leb_DecodeAttributeArray(in const cbt_Node node, in const float4x3 data);
